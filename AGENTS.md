# Agent Guidelines for Foundatio.Kafka
...
```bash
# Build
dotnet build Foundatio.Kafka.slnx

# Test
dotnet test Foundatio.Kafka.slnx

# Format code
dotnet format Foundatio.Kafka.slnx
```
...
```text
src
└── Foundatio.Kafka                    # Kafka provider implementations
    ├── Messaging                      # IMessageBus implementations for Kafka
    └── Extensions                     # Extension methods for DI
tests
└── Foundatio.Kafka.Tests              # Unit and integration tests
```
...
1. **Builds successfully**: `dotnet build Foundatio.Kafka.slnx` exits with code 0
2. **All tests pass**: `dotnet test Foundatio.Kafka.slnx` shows no failures
...
- Mirror the main code structure (e.g., `Messaging/` tests for `src/Foundatio.Kafka/Messaging/`)
...
# Specific test file
dotnet test --filter "FullyQualifiedName~KafkaMessageBusTests"
